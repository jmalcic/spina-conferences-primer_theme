    ...
    class ConferencesNavigationTest < ActionDispatch::IntegrationTest
      ...
      def assert_sponsors_for(conference, present: true)
        assert_select 'ul.structure', present do
          assert_select 'li', html: %r{/rails/active_storage/representations/},
                              count: conference.content(:sponsors).structure_items.joins(:structure_parts)
      ...
      def assert_submission_info_for(conference, present: true)
        assert_select 'div.flash', present do
          ...
          assert_select 'p', "Submit abstracts by #{I18n.localize(conference.content(:submission_date), format: :short)}"
      ...
      def assert_institution_logos_for(conference)
        assert_select 'li', html: %r{/rails/active_storage/representations/}, count: conference.institutions.where.not(logo: nil).count
